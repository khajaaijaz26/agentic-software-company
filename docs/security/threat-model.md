...
| Unexpected or lingering microphone capture | Recorder loads only after `Ctrl+R`/`/voice`; visible recording overlay; two-minute hard cap; Enter stops; Esc cancels; unmount cleanup; `--offline` rejects before microphone access; `voice doctor` enumerates names without opening an endpoint | OS/device drivers and a compromised dependency remain trusted; no hardware capture indicator can be guaranteed |
